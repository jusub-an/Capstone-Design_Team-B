...
class BodyMeasureEngine:
    ...
    - MediaPipe 포즈 추출 후 마스크 연산을 통해 실측 계산
    - 측면 사진이 추가 제공될 경우 둘레 보정 수행
    """
    ...
    def analyze(self, image_bgr: np.ndarray, user_height_cm: float, side_image_bgr: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        if armpits["left_armpit"] is None or armpits["right_armpit"] is None:
            ...
            if armpits["left_armpit"] is None: armpits["left_armpit"]  = {"x": float(lm11["x"]), "y": float(fallback_y)}
            if armpits["right_armpit"] is None: armpits["right_armpit"] = {"x": float(lm12["x"]), "y": float(fallback_y)}
    ...
    def _build_visual_images(
        ...
        _BG = np.array([245, 245, 245], dtype=np.float32)
        _FG = 60
