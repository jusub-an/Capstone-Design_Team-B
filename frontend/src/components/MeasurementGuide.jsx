...
import { FileText, Camera, MoveHorizontal, Shirt, User, Ruler, ClipboardList, AlertTriangle } from 'lucide-react';
import './MeasurementGuide.css';

const COMMON_TIPS = [
  { icon: <FileText size={18} />, text: 'A4 용지(21×29.7cm)를 옷과 겹치지 않게 옆에 반듯하게 놓아주세요.' },
  { icon: <Camera size={18} />, text: '카메라는 바닥과 수평이 되도록 위에서 정면으로 찍어주세요.' },
];

const TOP_TIPS = [
  { icon: <MoveHorizontal size={18} />, text: '겨드랑이 굴곡이 잘 보이도록 양소매를 살짝 벌려주세요.' },
  { icon: <Shirt size={18} />, text: '목 부분과 밑단이 구겨지지 않게 쫙 펴주세요.' },
];

const BOTTOM_TIPS = [
  { icon: <User size={18} />, text: '사타구니가 명확히 보이도록 두 다리를 겹치지 않게 A자 형태로 살짝 벌려주세요.' },
  { icon: <Ruler size={18} />, text: '허리선이 겹치거나 울지 않게 반듯하게 펴주세요.' },
];
...
function MeasurementGuide({ categoryType, isOpen, onClose }) {
  ...
  const categoryIcon = categoryType === 'Top' ? <Shirt size={18} /> : null;

  return (
    <div className={`mg-panel ${isOpen ? 'mg-panel-open' : ''}`}>
      <div className="mg-panel-inner">
        <div className="mg-panel-header">
          <span className="mg-panel-title" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><ClipboardList size={20} /> 촬영 가이드</span>
        ...
        <div className="mg-section">
          <div className="mg-section-title">
            <span className={`mg-badge ${categoryType === 'Top' ? 'mg-badge-top' : 'mg-badge-bottom'}`} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        <div className="mg-warn" style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
