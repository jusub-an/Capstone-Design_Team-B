...
function MyPage() {
  ...
  const refreshCart = () => {
    ...
    if (!emailStorage || emailStorage === 'No Email') return;
    fetch(`http://localhost:8000/api/cart/${encodeURIComponent(emailStorage)}`)
      .then(r => r.ok ? r.json() : [])
  ...
  const removeCartItem = async (itemId) => {
    ...
    await fetch(`http://localhost:8000/api/cart/${itemId}?user_email=${encodeURIComponent(emailStorage)}`, { method: 'DELETE' });
    refreshCart();
  };
